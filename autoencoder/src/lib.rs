...
#[pyfunction]
pub fn mug_confidence(app_path: String, rel_path_to_img: String) -> f32 {
    let image = match ImageReader::open(app_path.clone() + &rel_path_to_img) {
    ...
    let mut cnn: CNN = CNN::load_model(&(app_path + "/autoencoder/final_model.json")).unwrap();
